...
function dragDropMove(ev)
{
	...
	var leftX=e.clientX+90;
		dragContentDiv.style.left = e.clientX + scrollLeft-(document.body.clientWidth-leftX) + 'px';/*//700\\220*/
		dragContentDiv.style.top = e.clientY + scrollTop -220  + 'px';
	/*}*/
...
{	
	...
	var answerRight=["哇，你真聰明，全部答對了！","你真厲害，回答全部正確！"];
	var answerFalse=["回答錯誤，請再仔細檢查一次吧！","回答不正確，别灰心，再試一次吧！"];
	
	if(f==0){
		chkAnswer.innerHTML=answerRight[getRandom()];
	}else{
		chkAnswer.innerHTML=answerFalse[getRandom()];	
	}
	...
	for(var no=0;no<divs.length;no++){
		if(divs[no].className=='dragDropSmallBox'){
			...
			arrayOfAnswers[arrayOfAnswers.length] = divs[no];
				document.getElementById(divs[no].id).innerHTML+="<span id='correctAnswer"+divs[no].id+"' style='color:#000;float:right;margin:0;display:none;background-color:#FC3;width:15px;height:15px;padding-top:5px;'>&radic;</span>";

				document.getElementById(divs[no].id).innerHTML+="<span id='wrongAnswer"+divs[no].id+"' style='color:#000;float:right;margin:0;display:none;background-color:red;width:15px;height:18px;padding-top:2px;'>x</span>";
		}
